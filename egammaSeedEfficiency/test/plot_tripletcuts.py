...
def circle_curv(x1, y1, x2, y2, x3, y3):
    noflip = np.abs(x3 - x1) < np.abs(y3 - y1)
    #print("x1 = %.3f"%x1)
    #print("y1 = %.3f"%y1)
    #print("x2 = %.3f"%x2)
    #print("y2 = %.3f"%y2)
...
def curv_cut(hits_x, hits_y, hits_z, hits_l, dca_outer = False, hard_curv = False):
    dca_curv = 25.0
    hard_curv = 0.0328407224959
    for ihit in range(0,len(hits_l) - 2):
        for jhit in range(ihit,len(hits_l) - 1):
            for khit in range(jhit,len(hits_l)):
                #print("")
                if (dca_outer):
                    continue
                if (hard_curv):
...
def buildTriplets_mod(pixelDoubletTypes,pixelDoubletCoords):
    numberOfTriplets = 0
    max_rad = 25.0 
    dca_outer = 0.0328407224959
    tripletList = [[3,4,5,14],[6,7],[7,18],[8,9,10],[6,17],[7,18],[11],[12],[],[6,17],[7,18],[],[],[8,9,10],[],[11],[12],[],[],[]]
    for doublet1 in range(0,len(pixelDoubletTypes)):
        for doublet2 in range(doublet1,len(pixelDoubletTypes)):
            if pixelDoubletTypes[doublet2] in tripletList[pixelDoubletTypes[doublet1]]:
                #print("Doublet 1:")
                #print(pixelDoubletCoords[doublet1])
                #print("Doublet 2:")
                #print(pixelDoubletCoords[doublet2])
                x1 = pixelDoubletCoords[doublet1][0][0]
                y1 = pixelDoubletCoords[doublet1][0][1]
                x2 = pixelDoubletCoords[doublet2][0][0]
                y2 = pixelDoubletCoords[doublet2][0][1]
                x3 = pixelDoubletCoords[doublet2][1][0]
                y3 = pixelDoubletCoords[doublet2][1][1]
                curv_val = circle_curv(x1,y1,x2,y2,x3,y3)
                dca0_val = dca0(x1,y1,x2,y2,x3,y3)
                if (curv_val > max_rad):
                    continue
                if (dca0_val > np.abs(curv_val)*dca_outer): #This cut is wrong
                    continue
                numberOfTriplets = numberOfTriplets + 1

    return numberOfTriplets

def doubletCoord(hits_x,hits_y,hits_z,hits_l,cut = 2):
    coord_arr = []
    comb = []
    collection = [0]*20
    for ihit in range(0,len(hits_l)-1):
        for jhit in range(ihit+1,len(hits_l)):
            ...
            if( ((dr > maxr[combination]) or (dr<0)  or (abs((z * ro - mer * hits_z[idOut])) > z0cut * dr)) and cut >= 2):
                continue
            coord_arr.append([(hits_x[idIn],hits_y[idIn]),(hits_x[idOut],hits_y[idOut])])
    ...
    if(hasCombination and hasAtLeastOne):
        ...
        for i in range(len(indx_arr)):
            coord_arr.remove(coord_arr[indx_arr[i] - i])
        comb.remove(19) #How do I handle this??
...
def main(options, paths):
    ...
    #Histograms for comparing samples with and without triplet cuts only
    nTriplet_NoCuts = r.TH1F("nTriplet_NoCuts",";Number of Pixel triplets ",10,0,10)
    nTriplet_Cuts = r.TH1F("nTriplet_Cuts",";Number of Pixel triplets ",10,0,10)
    ...
    for event in f.Get(tree+"/tree"):
        ...
        for electron in range(0,len(event.nRecoHitLayersBPIX)):
            if(event.isMatched[electron] or noGsfEleMatching):
                ...
                #print("Doublet Quality:")
                #print(doubletsQuality)
                #print(len(doubletsQuality))
                coords = doubletCoord(event.recHit_x[electron],event.recHit_y[electron],event.recHit_z[electron],event.recHit_l[electron])
                #print("Doublet Coordinates:")
                #print(coords)
                #print(doubletCoord(event.recHit_x[electron],event.recHit_y[electron],event.recHit_z[electron],event.recHit_l[electron]))
                ...
                coords_z = doubletCoord(event.recHit_x[electron],event.recHit_y[electron],event.recHit_z[electron],event.recHit_l[electron],1)
                for pairNum in range(20):
                ...
                coords_b = doubletCoord(event.recHit_x[electron],event.recHit_y[electron],event.recHit_z[electron],event.recHit_l[electron],0)
                for pairNum in range(20):
                ...
                # Build triplets with cuts
                triplets_cut_all = buildTriplets_mod(doubletsQuality,coords)
                triplets_cut_z = buildTriplets_mod(doubletsQuality_z,coords_z)
                triplets_cut = buildTriplets_mod(doublets_b,coords_b)
                ...
                nTriplets.Fill(triplets_cut,1.)
                nTripletszCut.Fill(triplets_cut_z,1.)
                nTripletsQualCut.Fill(triplets_cut_all,1.)
                dnTriplets.Fill(triplets_cut - triplets_cut_all,1.)

                nTriplet_NoCuts.Fill(triplets, 1.)
                nTriplet_Cuts.Fill(triplets_cut, 1.)
                if (triplets != triplets_cut):
                    print("Different number of triplets")
                    print(triplets)
                    print(triplets_cut)
    ...
    #Compare number of triplets with and without triplet cuts
    c = r.TCanvas("c", "canvas", 900, 700)
    c.cd()
    nTriplet_NoCuts.SetLineColor(r.kBlack)
    nTriplet_NoCuts.Draw("HIST")
    nTriplet_Cuts.SetLineColor(r.kRed)
    nTriplet_Cuts.Draw("HIST same")
    ltemp = r.TLegend(.4, .72, .59, .85)
    ltemp.AddEntry(nTriplet_NoCuts,"No Triplet Cuts Applied","l") 
    ltemp.AddEntry(nTriplet_Cuts,"Triplet Cuts Applied","l") 
    ltemp.Draw("same")
    c.SaveAs(dir+'/nTripletCut_Comp.png')
